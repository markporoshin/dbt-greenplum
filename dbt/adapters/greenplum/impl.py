...
@dataclass
class GreenplumIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["GreenplumIndexConfig"]:
...
class GreenplumConfig(AdapterConfig):
    ...
    indexes: Optional[List[GreenplumIndexConfig]] = None
...
class GreenplumAdapter(SQLAdapter):
    ...
    def _get_catalog_schemas(self, manifest):
        # greenplum only allow one database (the main one)
