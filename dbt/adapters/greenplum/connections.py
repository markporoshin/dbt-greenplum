...
class GreenplumCredentials(Credentials):
    ...
    user: str
    port: Port
    password: str  # on greenplum the password is mandatory
...
class GreenplumConnectionManager(SQLConnectionManager):
    TYPE = 'greenplum'
    ...
    def open(cls, connection):
        ...
        kwargs = {}
        # we don't want to pass 0 along to connect() as greenplum will try to
        ...
        if search_path is not None and search_path != "":
            # see https://greenplumql.org/docs/9.5/libpq-connect.html
        ...
        try:
            ...
            logger.debug(
                "Got an error when attempting to open a greenplum " "connection: '{}'".format(e)
            )
